<template>
  <div
    class="sticky top-0 z-10 flex-shrink-0 w-full bg-gray-200 bg-opacity-50 h-15 dark:bg-transparent"
  >
    <div class="flex items-center h-full px-8">
      ...
      <div>
        ...
        >
          <figure
            class="relative inline-block w-10 h-10 overflow-hidden"
            title="blazinskip"
            data-testid="user-widget-avatar"
            style="width: 28px; height: 28px"
          >
            <div
              class="flex flex-col items-center justify-center w-full h-full bg-gray-700 rounded-full"
            >
              <svg
                width="16"
                height="16"
          ...
          <span v-if="user" class="flex px-2">{{ user.display_name }}</span>
        </button>
      </div>
    </div>
  </div>
